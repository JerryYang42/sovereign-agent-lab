...
TASK_A_NOTES = "Flyer generation succeeded but returned placeholder mode (no live image model configured)."
...
Because the tool returns a structured success response with a labelled placeholder fallback instead of throwing, the agent loop keeps running even when the live image provider is unavailable.
"""
...
The first tool response showed that The Bow Bar's capacity is 80, which is too small. So, they moved on to check The Albanach next.
"""
...
SCENARIO_3_RESPONSE = "I don't have access to real-time train schedules or transportation data. For accurate timing, it recommended checking National Rail, Trainline/Citymapper, or the operator site (e.g., LNER)."

# Would this behaviour be acceptable in a real booking assistant? Min 30 words.
SCENARIO_3_ACCEPTABLE = """
Mostly yes. The assistant correctly avoided inventing train times and clearly stated scope limits, then redirected the user to authoritative sources. In a real booking assistant, this is safer than hallucinating. It could be improved by offering to open a trusted timetable link directly.
"""
...
TASK_D_COMPARISON = """
The LangGraph Mermaid shows a minimal runtime loop: start -> agent, optional tool call, then back to agent until it decides to end. Routing is implicit and decided step-by-step by the model. In flows.yml, Rasa declares explicit, named flows (confirm_booking, handle_out_of_scope) with deterministic slot-collection/action steps, so behavior is more auditable and constrained after flow selection.
"""
...
MOST_SURPRISING = """
The most unexpected behavior was in Task A: even though the prompt explicitly asked to check both The Albanach and The Haymarket Vaults, the tool trace only checked The Albanach, then immediately proceeded to cost, weather, and flyer generation. It still solved the booking objective, but it did not fully follow the literal instruction to check two venues, which is a subtle compliance gap.
"""