...
install: check-uv check-env ## Set up main environment (Python 3.14, exercises 1/2/4)
	...
	@echo "  Python: $(shell $(SSL_CERT_PREFIX)$(UV) run python --version 2>/dev/null || echo unknown)"
...
install-rasa: check-uv ## Set up Rasa Pro environment (Python 3.10, Exercise 3 only)
	...
	@echo "  Rasa version: $(shell cd $(RASA_DIR) && $(SSL_CERT_PREFIX)$(UV) run rasa --version 2>/dev/null | head -1 || echo unknown)"
